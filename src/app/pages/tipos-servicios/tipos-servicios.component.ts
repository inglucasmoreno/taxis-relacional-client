...
export class TiposServiciosComponent implements OnInit {
  ...
  constructor(private tiposServiciosService: TiposServiciosService,
    private authService: AuthService,
    private alertService: AlertService,
    private dataService: DataService) { }

  ngOnInit(): void {
    this.dataService.ubicacionActual = 'Dashboard - Tipos de servicio';
  ...
  // Traer datos del tipo
  getTipo(tipo: any): void {
    this.alertService.loading();
    this.idTipo = tipo.id;
    this.tipoSeleccionado = tipo;
    this.tiposServiciosService.getTipo(tipo.id).subscribe(({ tipo }) => {
  ...
  listarTipos(): void {
    ...
    this.tiposServiciosService.listarTipos(parametros)
      .subscribe(({ tipos }) => {
        this.tipos = tipos;
  ...
  nuevoTipo(): void {
    ...
    this.tiposServiciosService.nuevoTipo(data).subscribe(() => {
      this.listarTipos();
    }, ({ error }) => {
  ...
  actualizarTipo(): void {
    ...
    this.tiposServiciosService.actualizarTipos(this.idTipo, data).subscribe(() => {
      this.listarTipos();
    }, ({ error }) => {
      this.alertService.errorApi(error.message);
    });
  ...
  actualizarEstado(tipo: any): void {
    ...
    this.alertService.question({ msg: '¿Quieres actualizar el estado?', buttonText: 'Actualizar' })
      .then(({ isConfirmed }) => {
        if (isConfirmed) {
          this.alertService.loading();
          this.tiposServiciosService.actualizarTipos(id, { activo: !activo }).subscribe(() => {
            this.alertService.loading();
            this.listarTipos();
          }, ({ error }) => {
            this.alertService.close();
