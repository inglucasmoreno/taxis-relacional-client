...
@Component({
  selector: 'app-vehiculos-marcas',
  templateUrl: './vehiculos-marcas.component.html',
  styles: [
  ]
})
export class VehiculosMarcasComponent implements OnInit {
  ...
  constructor(private vehiculosMarcasService: VehiculosMarcasService,
    private authService: AuthService,
    private alertService: AlertService,
    private dataService: DataService) { }

  ngOnInit(): void {
    this.dataService.ubicacionActual = 'Dashboard - Vehiculos - Marcas';
  ...
  getMarca(marca: any): void {
    ...
    this.vehiculosMarcasService.getMarca(marca.id).subscribe(({ marca }) => {
      console.log(marca);
  ...
  listarMarcas(): void {
    ...
    this.vehiculosMarcasService.listarMarcas(parametros)
      .subscribe(({ marcas }) => {
        this.marcas = marcas;
  ...
  nuevaMarca(): void {
    ...
    this.vehiculosMarcasService.nuevaMarca(data).subscribe(() => {
      this.listarMarcas();
    }, ({ error }) => {
  ...
  actualizarMarca(): void {
    ...
    this.vehiculosMarcasService.actualizarMarcas(this.idMarca, data).subscribe(() => {
      this.listarMarcas();
    }, ({ error }) => {
      this.alertService.errorApi(error.message);
    });
  ...
  actualizarEstado(marca: any): void {
    ...
    this.alertService.question({ msg: '¿Quieres actualizar el estado?', buttonText: 'Actualizar' })
      .then(({ isConfirmed }) => {
        if (isConfirmed) {
          this.alertService.loading();
          this.vehiculosMarcasService.actualizarMarcas(id, { activo: !activo }).subscribe(() => {
            this.alertService.loading();
            this.listarMarcas();
          }, ({ error }) => {
            this.alertService.close();
