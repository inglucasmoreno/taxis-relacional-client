...
@Component({
  selector: 'app-vehiculos-modelos',
  templateUrl: './vehiculos-modelos.component.html',
  styles: [
  ]
})
export class VehiculosModelosComponent implements OnInit {
  ...
  constructor(
    private vehiculosModelosService: VehiculosModelosService,
  ...
  ngOnInit(): void {
    this.dataService.ubicacionActual = 'Dashboard - Vehiculos - Modelos';
  ...
  getModelo(modelo: any): void {
    ...
    this.vehiculosModelosService.getModelo(modelo.id).subscribe(({ modelo }) => {
  ...
  listarModelos(): void {
    ...
    this.vehiculosModelosService.listarModelos(parametros)
      .subscribe(({ modelos }) => {
  ...
  nuevoModelo(): void {
    ...
    this.vehiculosModelosService.nuevoModelo(data).subscribe(() => {
      this.listarModelos();
    }, ({ error }) => {
  ...
  actualizarModelo(): void {
    ...
    this.vehiculosModelosService.actualizarModelos(this.idModelo, data).subscribe(() => {
      this.listarModelos();
    }, ({ error }) => {
      this.alertService.errorApi(error.message);
    });
  ...
  actualizarEstado(modelo: any): void {
    ...
    this.alertService.question({ msg: '¿Quieres actualizar el estado?', buttonText: 'Actualizar' })
      .then(({ isConfirmed }) => {
        if (isConfirmed) {
          this.alertService.loading();
          this.vehiculosModelosService.actualizarModelos(id, { activo: !activo }).subscribe(() => {
            this.alertService.loading();
            this.listarModelos();
          }, ({ error }) => {
            this.alertService.close();
